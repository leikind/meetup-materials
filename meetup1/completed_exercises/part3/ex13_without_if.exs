...
defmodule RomanNumeral do
  ...
  def convert(roman_numeral)  do
    roman_numeral |> String.codepoints |> Enum.map(&roman_digit_to_decimal/1) |> _convert
  end

  defp _convert([n]), do: n
  defp _convert([n1 | tail = [n2 | _]]) when n1 < n2, do: _convert(tail) - n1
  defp _convert([n | tail]), do: _convert(tail) + n
